...
const STEPS = ["ticker", "interval", "timeframe", "strategy", "amount", "review"] as const;
...
function Pill({ selected, onClick, children }: { selected: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      ...
      onClick={onClick}
      className={clsx(
        "px-4 py-2 rounded-lg border text-[13px] font-medium transition",
        selected
          ? "border-emerald-500 bg-emerald-500/12 text-emerald-400 font-bold"
          : "border-zinc-700/50 bg-zinc-800/30 text-zinc-400 hover:border-zinc-600 hover:text-zinc-200"
      )}
    >
      {children}
    </button>
...
export default function BacktestBuilder({ onSubmit, onClose }: { onSubmit: (prompt: string) => void; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div
        className="w-full max-w-[460px] rounded-2xl border border-emerald-500/15 bg-gradient-to-b from-zinc-900 to-zinc-950 shadow-2xl shadow-black/60 overflow-hidden"
      ...
      >
        ...
        <div className="px-6 pt-5 pb-3 border-b border-zinc-800/60">
          <div className="flex items-center justify-between mb-1">
            ...
            <button type="button" onClick={onClose} className="p-1 rounded-lg hover:bg-zinc-800 transition text-zinc-500 hover:text-zinc-300">
              <X className="h-4 w-4" />
            </button>
          </div>
          ...
          <div className="flex gap-1 mt-4">
            {STEPS.map((s, i) => (
              <div key={s} className="flex-1 flex flex-col items-center gap-1.5">
                <div className={clsx("h-[3px] w-full rounded-full transition-colors", i <= step ? "bg-emerald-400" : "bg-zinc-800")} />
                <span className={clsx("text-[9px] font-medium uppercase tracking-wide transition-colors", i <= step ? "text-emerald-400" : "text-zinc-700", i === step && "font-bold")}>
                  {STEP_LABELS[i]}
                </span>
              </div>
            ))}
        ...
        <div className="px-6 py-5 min-h-[260px]">
          ...
          {currentKey === "ticker" && (
            <div>
              <p className="text-[13px] text-zinc-500 mb-3">Select a ticker to backtest</p>
          ...
          {currentKey === "interval" && (
            <div>
              <p className="text-[13px] text-zinc-500 mb-3">Choose chart interval</p>
              <div className="flex flex-wrap gap-2">
          ...
          {currentKey === "timeframe" && (
            <div>
              <p className="text-[13px] text-zinc-500 mb-3">How far back to test?</p>
              <div className="flex flex-wrap gap-2">
                {TIMEFRAMES.map((tf) => (
          ...
          {currentKey === "strategy" && (
            <div>
              <p className="text-[13px] text-zinc-500 mb-3">Pick a trading strategy</p>
                ...
                {STRATEGIES.map((s, i) => (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className={clsx("text-[13px] font-bold", config.strategy === i ? "text-emerald-400" : "text-white")}>{s.name}</span>
                    </div>
                    <p className="text-[11px] text-zinc-500 leading-relaxed ml-7">{s.desc}</p>
                  </button>
                ))}
              </div>
            </div>
          ...
          {currentKey === "amount" && (
            <div>
              <p className="text-[13px] text-zinc-500 mb-3">Enter backtest amount</p>
              <div className="flex items-center rounded-xl border border-zinc-700/50 bg-zinc-800/30 px-4 mb-3">
                ...
                <input
                  ...
                  className="flex-1 bg-transparent border-0 outline-none text-white text-xl font-bold py-3 placeholder:text-zinc-700"
                />
              </div>
              <div className="flex flex-wrap gap-2">
              ...
              <p className="text-[11px] text-zinc-600 mt-3">Min $100 · Max $100,000</p>
            </div>
          )}
          ...
          {currentKey === "review" && (
            <div>
              <p className="text-[13px] text-zinc-500 mb-3">Review your backtest configuration</p>
              {[
                ...
                <div key={row.label} className="flex items-center justify-between py-2.5 border-b border-zinc-800/50">
                  <div>
                    <span className="text-[10px] text-zinc-500 uppercase tracking-wide font-semibold">{row.label}</span>
                    <div className="text-sm font-bold text-white mt-0.5">{row.value}</div>
                  </div>
                  <button
                    type="button"
                    onClick={() => setStep(row.stepIdx)}
                    className="px-3 py-1 rounded-lg border border-zinc-700/50 bg-zinc-800/30 text-emerald-400 text-[11px] font-semibold hover:bg-zinc-700/50 transition"
                  >
                    Edit
                  </button>
                </div>
              ...
              {selectedStrategy && (
                <div className="mt-3 px-3 py-2.5 rounded-lg border border-emerald-500/15 bg-emerald-500/5">
                  <span className="text-[10px] text-emerald-400 font-bold uppercase tracking-wide">Strategy Logic</span>
                  <p className="text-[12px] text-zinc-400 leading-relaxed mt-1">{selectedStrategy.desc}</p>
                </div>
              )}
            </div>
          )}
        ...
        {/* Footer */}
        <div className="px-6 pb-5 pt-3 border-t border-zinc-800/50 flex gap-3">
          {step > 0 && (
            <button
              type="button"
              onClick={handleBack}
              className="flex-1 py-3 rounded-xl border border-zinc-700/50 text-zinc-400 text-sm font-semibold hover:bg-zinc-800 transition"
            >
              ← Back
            </button>
          )}
          <button
            ...
            className={clsx(
              ...
              currentKey === "review" && canProceed
                ? "bg-gradient-to-r from-emerald-500 to-emerald-600 text-zinc-950 shadow-lg shadow-emerald-500/25 hover:shadow-emerald-500/40"
                : canProceed
                  ? "bg-emerald-500/15 text-emerald-400 hover:bg-emerald-500/25"
                  : "bg-zinc-800/30 text-zinc-600 cursor-not-allowed"
            )}
          >
            {currentKey === "review" ? "🚀 Load Backtest" : "Continue →"}
          </button>
        </div>
      </div>
    </div>
